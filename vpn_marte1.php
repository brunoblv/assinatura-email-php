...
<form action="http://10.75.32.129/ass/vpn_marte1.php" autocomplete="ON" method="get" name="form">Pesquisar: <input maxlength="50" name="contato" size="50" type="text" value="<?php echo $_GET['contato']; ?>"><input type="submit" value="Pesquisar" />
<a href="http://10.75.32.129/ass/pesquisa.xls.php?contato=" target="_self"><img class="" src="http://10.75.32.129/wp-content/uploads/2017/06/excel-xls-icon.png" alt="" width="37" height="49" /></a></form>
...
if (($connect=@ldap_connect($ldap_server))) {
	if (($bind=@ldap_bind($connect, $auth_user, $auth_pass))) {
		...
		$pessoa = array_merge($smdu, $sel, $ilume);

		$count = count($pessoa);
		Echo "SMDU: " . $count1 . " usuários<br> SEL: " . $count2 . " usuários<br> ILUME: " . $count3 . " usuários<br><br>";
		//sort($pessoa);
			for ($i=0; $i < $count; $i++){
		...
		$Tipo = substr($pessoa[$i][usuario], 0,-6);
 		...
 		if (!($pessoa[$i][ultimologon] == "Nunca")) {
				if ($pessoa[$i][diasalto] >= "60") { 
					echo "<td><font color=red><b><a href='http://10.75.32.129/ass/pesquisa.uni.php?id=".$pessoa[$i][usuario]."&Nome=".$pessoa[$i][nome]."' onclick='abre();' target='verifica_IFRAME'>". $pessoa[$i][ultimologon]." - ". $pessoa[$i][diasalto]." dias</b></font></td>";
			...
			echo "<td><font color=red><b><a href='http://10.75.32.129/ass/pesquisa.uni.php?id=".$pessoa[$i][usuario]."&Nome=".$pessoa[$i][nome]."' onclick='abre(); 'target='verifica_IFRAME'>". $pessoa[$i][ultimologon]."</b></font></td>";
			echo "<td><font color=red>Atenção</font></td>";
			//echo "<td><center><input type=checkbox name=\"".$pessoa[$i][usuario]." - ".$pessoa[$i][nome]." - ".$pessoa[$i][Email]." \" value= \"(  ) Excluir\"><input type=button value=Excluir></center></td>";
