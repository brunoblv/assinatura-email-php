...
<html lang="en">
<head>
  <title>Lista de todos usuários de SMDU.</title>
  <meta charset="utf-8">
...
<center><h1>Lista de todos usuários de SMDU.</h1>

<form action="http://intranet.smdu.pmsp/ass/smdu_todos.php" autocomplete="ON" method="get" name="form">Pesquisar: <input maxlength="50" name="contato" size="50" type="text" value="<?php echo $_GET['contato']; ?>"><input type="submit" value="Pesquisar" />
...
$filter2 = "(&(objectClass=user)(objectCategory=person)(|(cn=$pesquisa)(samaccountname=$pesquisa))(!(userAccountControl:1.2.840.113556.1.4.803:=2))(memberOf=CN=G_X_VPN_PREFEITURA,OU=Groups,OU=PRODAM,DC=rede,DC=sp))";
...
if (($connect=@ldap_connect($ldap_server))) {
	if (($bind=@ldap_bind($connect, $auth_user, $auth_pass))) {
		...
		if (($search=@ldap_search($connect, $base_dn, $filter2))){
...
ldap_close($connect);
		...
		Echo "Foram encontrados " . $result . " pessoas nesta pesquisa.<br>";
			...
			echo "<tr><td>".$pessoa[$i][usuario]."</td>";
				...
				echo "<td><a href=mailto:".$pessoa[$i][Email].">".$pessoa[$i][Email]."</a>";
